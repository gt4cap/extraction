...
url = """https://finder.creodias.eu/resto/api/collections/Sentinel1/search.atom?maxRecords=2000&startDate={}&completionDate={}&productType={}&sensorMode=IW&timeliness=NRT-3h&sortParam=startDate&sortOrder=descending&status=all&geometry={}&dataset=ESA-DATASET"""
...
ptype = sys.argv[3]   # GRD or SLC
card = sys.argv[4]    # GR or SL
...
insertSql = """
    INSERT into dias_check (obstime, reference, sensor, card, footprint) 
...
getMetadataSql = """
    select card, sensor, count(*), min(obstime), max(obstime) from dias_check 
